...
class SimpleOopProper
{
    ...
    function __construct()
    {
        ...
        try {
            ...
            // the more proper is to send exception to a file list / or table and send to end 
    ...
    function connect()
    {
        // init value , this may diff with your setup. the proper is to create a file outside 
        ...
        try {
            $this->connection = new mysqli(
                $this->connectionString->getServerName(),
                $this->connectionString->getUserName(),
                $this->connectionString->getPassword(),
                $this->connectionString->getDatabase()
            );
        } catch (Exception $exception) {
    ...
    function search()
    {
        // you don't need to commit work here ya !
        try {
            $result = $this->connection->query("SELECT * FROM person WHERE name like '%" . $this->search . "%' OR  age like '%" . $this->search ."%'");
            $data = [];
            while (($row = $result->fetch_assoc()) == TRUE) {
                $data[] = $row;
            }
    ...
    function update()
    {
        ...
        $var1 = $this->model->getName();
        $var2 = $this->model->getAge();
        // the more proper is to hash the id  , so hijacker wouldn't know the value , doesn't 
        // random encryption
        ...
        if (strlen($var1) > 0 && $var2 > 0 && $var3 > 0) {
            ...
            ///
            $statement = $this->connection->prepare("UPDATE person SET name = ?,age = ? WHERE  
personId = ?");
            // s -> string, i -> integer , d -  double , b - blob
...
$mode_get = filter_input(INPUT_GET, "mode_get", FILTER_SANITIZE_FULL_SPECIAL_CHARS);
...
if ($mode_get) {
    try {
        switch ($mode_get) {
            ...
            default:
                throw new Exception(
                    ReturnCode::ACCESS_DENIED_NO_MODE,
