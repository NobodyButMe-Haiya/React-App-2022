...
<table class="table table-striped">
    ...
    while (($row = $result->fetch_assoc()) == TRUE) { ?>
        <tr>
            ...
            <td>
                <input style="text-align: right" class="form-control" id="<?php echo 
$row["personId"]; ?>_age"
                       type="text" value="<?php echo $row["age"]; ?>"/>
            </td>
            <td>
                <div class="btn-group" role="group" aria-label="Form Button">
                    <button type="button" class="btn btn-warning" onclick="updateRecord(<?php 
echo $row["personId"]; ?>)">
...
<script>
    // at here we try to be native as possible and you can use url to ease change the which one 
    let url = "api.php";
...
?>?notification=createSuccess";
                        ...
                        // popup saying error
                        console.log("error message : " + obj.message + "Error Code : " + 
obj.code)
                    }
                } else {
        ...
        xmlHttpRequest.send("&mode=create&name=" + document.getElementById("name").value + 
...
obj.code)
        ...
        xmlHttpRequest.send("&mode=update&&personId=" + personId + "&name=" + 
document.getElementById(personId + "_name").value + "&age=" + document.getElementById(personId + 
"_age").value);
